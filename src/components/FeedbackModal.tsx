  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Esto nos dirá si el problema sigue siendo el 401
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 font-sans">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden text-left">
            <div className="bg-slate-900 p-6 border-b-4 border-green-500 text-white">
              <h3 className="font-mono font-bold text-lg">DEVELOPER_CONSOLE.EXE</h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <input type="text" required placeholder="Asunto..." className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-slate-800 outline-none" value={formData.asunto} onChange={(e) => setFormData({...formData, asunto: e.target.value})} />
              <textarea required rows={3} placeholder="Detalles..." className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-slate-800 outline-none resize-none" value={formData.descripcion} onChange={(e) => setFormData({...formData, descripcion: e.target.value})} />
              <button type="submit" disabled={loading} className="w-full bg-slate-900 text-green-400 font-mono font-bold py-4 rounded-xl border-b-4 border-green-600">
