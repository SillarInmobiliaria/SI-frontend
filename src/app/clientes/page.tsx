    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 font-sans text-gray-800 flex flex-col">
      ...
      <div className="flex flex-1 relative">
          ...
          <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
                        ...
                        {clientesFiltrados.map((c: any) => {
                            ...
                            return (
                                <tr key={c.id} className="group bg-white hover:bg-indigo-50/30 shadow-sm hover:shadow-md transition-all rounded-2xl">
                                    <td className="rounded-l-2xl border-y border-l border-gray-100 py-4 pl-4"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full text-white font-bold flex items-center justify-center shadow-lg ${c.tipo === 'CLIENTE' ? 'bg-gradient-to-br from-green-500 to-emerald-600' : 'bg-gradient-to-br from-orange-400 to-amber-500'}`}>{c.nombre.charAt(0)}</div><div className={`text-xs font-bold px-3 py-1 rounded-full ${c.tipo === 'CLIENTE' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>{c.tipo === 'CLIENTE' ? 'CLIENTE' : 'INTERESADO'}</div></div></td>
            ...
            {/* MODAL NUEVO REGISTRO Y OTROS... (MANTENER TODO EL CÓDIGO DE MODALES QUE YA TENÍAS) */}
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto relative">
                        ...
                        <form onSubmit={handleSubmit(onSubmitCliente)} className="p-8 bg-gradient-to-br from-gray-50 to-blue-50">
                            {/* ... (CONTENIDO DEL FORMULARIO IGUAL QUE EL CÓDIGO ANTERIOR) ... */}
                            {/* RECUERDA: COPIA EL FORMULARIO COMPLETO QUE YA FUNCIONABA CON LA TARJETA Y EL OJITO */}
                                        ...
                                        {propiedadSeleccionada && (
                                            <div className="mt-6 bg-blue-50 border-2 border-blue-200 rounded-2xl p-6 relative shadow-md">
                                                ...
                                                {propiedadSeleccionada.tipo === 'Departamento' && (
                                                    <div className="bg-white p-3 rounded-xl border border-blue-100 w-fit">
                                                        <span className="text-xs font-bold text-blue-800 uppercase">MANTENIMIENTO: </span>
                                                        <span className="text-sm font-bold text-gray-700">{propiedadSeleccionada.mantenimiento && Number(propiedadSeleccionada.mantenimiento) > 0 ? `S/ ${Number(propiedadSeleccionada.mantenimiento).toLocaleString('es-PE')}` : 'No'}</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
            ...
            {/* OTROS MODALES (VER DETALLE CLIENTE, REQUERIMIENTO RÁPIDO, SEGUIMIENTO RÁPIDO) */}
