...
import { 
  ...
  FaInstagram, FaTiktok, FaGlobe, FaHandshake,
...
export default function PropiedadDetallePage() {
  ...
  const getLinkConfig = (url: string) => {
    ...
    if (link.includes('sillarinmobiliaria.pe')) return { label: 'Sillar Inmobiliaria', icon: <FaGlobe className="text-blue-600"/>, color: 'bg-blue-50 border-blue-100 text-blue-700' };
    if (link.includes('instagram.com')) return { label: 'Instagram', icon: <FaInstagram className="text-pink-600"/>, color: 'bg-pink-50 border-pink-100 text-pink-700' };
    if (link.includes('tiktok.com')) return { label: 'TikTok', icon: <FaTiktok className="text-black"/>, color: 'bg-gray-100 border-gray-200 text-gray-900' };
    if (link.includes('urbania.pe')) return { label: 'Urbania', icon: <FaLink className="text-red-600"/>, color: 'bg-red-50 border-red-100 text-red-700' };
  ...
  const guardarCambios = async () => {
    ...
    try {
        await api.put(`/propiedades/${id}`, { observaciones: JSON.stringify(observaciones), ...estadosDocs });
  ...
  const getIconoSemaforo = (estado: boolean | null) => {
    if (estado === true) return <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center shadow-md"><FaCheckCircle className="text-white text-sm"/></div>;
    if (estado === null) return <div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center shadow-md"><FaExclamationCircle className="text-white text-sm"/></div>;
    return <div className="w-8 h-8 rounded-full bg-red-500 flex items-center justify-center shadow-md"><FaTimesCircle className="text-white text-sm"/></div>;
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 lg:p-8 min-h-[400px]">
                    {activeTab === 'informacion' && (
                        <div className="space-y-8 animate-fade-in">
                            {images.length > 0 && (
                                <div className="space-y-4">
                                    <div className="group h-[450px] rounded-2xl overflow-hidden relative bg-gray-100 border border-gray-200 flex items-center justify-center">
                                        ...
                                        {images.length > 1 && (
                                            <>
                                                <button onClick={handlePrevImage} className="absolute left-4 bg-white/80 hover:bg-white text-indigo-900 p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all active:scale-95"><FaChevronLeft className="text-xl"/></button>
                                                <button onClick={handleNextImage} className="absolute right-4 bg-white/80 hover:bg-white text-indigo-900 p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all active:scale-95"><FaChevronRight className="text-xl"/></button>
                                            </>
                                        )}
                                        <div className="absolute bottom-4 right-4 bg-black/60 text-white px-3 py-1 text-xs font-black rounded-lg backdrop-blur-md">{currentImageIndex + 1} / {images.length}</div>
                                    </div>
                                    <div className="flex gap-3 overflow-x-auto pb-2 scroll-smooth">
                                        {images.map((img:string, idx:number) => (
                                            <img key={idx} src={getFullImageUrl(img)} onClick={() => setCurrentImageIndex(idx)} crossOrigin="anonymous" className={`w-20 h-20 object-cover rounded-xl cursor-pointer border-2 transition-all flex-shrink-0 ${currentImageIndex===idx ? 'border-indigo-600 scale-95' : 'border-transparent hover:border-gray-300 opacity-60 hover:opacity-100'}`}/>
                                        ))}
                                    </div>
                                </div>
                            )}
                            ...
                            {esProyecto && (
                                <div className="bg-indigo-50 rounded-3xl p-8 border border-indigo-100 shadow-sm animate-fade-in">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-sm">
                                        <div className="bg-white p-4 rounded-2xl border border-indigo-200 flex items-center gap-4">
                                            ...
                                            <div><p className="text-[10px] text-gray-400 font-black uppercase tracking-tighter leading-none">Inicio de Obra</p><p className="font-black text-indigo-900 mt-1">{propiedad.fechaInicioProyecto || 'Por definir'}</p></div>
                                        </div>
                                        <div className="bg-white p-4 rounded-2xl border border-indigo-200 flex items-center gap-4">
                                            <div className="p-3 bg-indigo-100 rounded-xl text-indigo-600"><FaTools size={20}/></div>
                                            <div><p className="text-[10px] text-gray-400 font-black uppercase tracking-tighter leading-none">Ejecución</p><p className="font-black text-indigo-900 mt-1">{propiedad.tiempoEjecucion || 'No especificado'}</p></div>
                                        </div>
                                    </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-6 border-t border-b border-gray-100 text-center">
                                <div><FaRulerCombined className="mx-auto text-3xl text-emerald-500 mb-2"/><p className="font-black text-xl">{propiedad.area || 0} m²</p><p className="text-xs font-black text-gray-400 uppercase">Total</p></div>
                            ...
                            {propiedad.descripcion && (
                                <div><h3 className="text-lg font-black text-gray-800 mb-3 flex items-center gap-2 uppercase tracking-tighter"><FaTag className="text-indigo-500"/> Descripción</h3><p className="text-gray-600 leading-relaxed whitespace-pre-line text-lg">{propiedad.descripcion}</p></div>
                            )}
                    ...
                    {activeTab === 'ubicacion' && (
                        <div className="animate-fade-in flex flex-col items-center justify-center min-h-[400px]">
                            <h3 className="text-2xl font-black text-gray-800 mb-2">{propiedad.ubicacion}</h3><p className="text-gray-500 text-lg mb-6">{propiedad.direccion}</p>
                    ...
                    {activeTab === 'legal' && isAdmin && (
                        <div className="animate-fade-in space-y-8">
                            ...
                            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                <h3 className="text-lg font-black text-gray-800 mb-4 flex items-center gap-2 uppercase tracking-tighter"><FaHandshake className="text-blue-600"/> Detalles del Contrato</h3>
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                                        ...
                                        <div className={`badge ${propiedad.exclusiva ? 'badge-success text-white' : 'badge-ghost text-gray-500'} font-black`}>{propiedad.exclusiva ? 'SÍ' : 'NO'}</div>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Renovable</p>
                                        <div className={`badge ${propiedad.renovable ? 'badge-info text-white' : 'badge-ghost text-gray-500'} font-black`}>{propiedad.renovable ? 'SÍ' : 'NO'}</div>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">IGV</p>
                                        <div className={`badge ${propiedad.incluyeIgv ? 'badge-secondary text-white' : 'badge-ghost text-gray-500'} font-black`}>{propiedad.incluyeIgv ? 'SÍ' : 'NO'}</div>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Inicio</p>
                                        <p className="font-black text-gray-800 text-sm">{propiedad.inicioContrato ? propiedad.inicioContrato.split('-').reverse().join('/') : '-'}</p>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Fin</p>
                                        <p className="font-black text-gray-800 text-sm">{propiedad.finContrato ? propiedad.finContrato.split('-').reverse().join('/') : '-'}</p>
                                    </div>
                                    {Number(propiedad.vigilancia) > 0 && (
                                        <div className="col-span-2 md:col-span-5 bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex flex-col md:flex-row items-center justify-between gap-2">
                                            <span className="text-xs text-emerald-700 font-black uppercase tracking-widest flex items-center gap-2"><FaShieldAlt className="text-emerald-500 text-lg"/> Pago Vigilancia</span>
                                            <span className="font-black text-emerald-900 text-xl">{propiedad.monedaVigilancia === 'USD' ? '$' : 'S/'} {Number(propiedad.vigilancia).toLocaleString('es-PE', { minimumFractionDigits: 2 })}</span>
                                        </div>
                                    )}
                                    {propiedad.modalidad === 'Alquiler' && Number(propiedad.mantenimiento) > 0 && (
                                        <div className="col-span-2 md:col-span-5 bg-blue-50 p-4 rounded-xl border border-blue-100 flex flex-col md:flex-row items-center justify-between gap-2">
                                            <span className="text-xs text-blue-700 font-black uppercase tracking-widest flex items-center gap-2"><FaTools className="text-blue-500 text-lg"/> Mantenimiento Edificio</span>
                                            <span className="font-black text-blue-900 text-xl">{propiedad.monedaMantenimiento === 'USD' ? '$' : 'S/'} {Number(propiedad.mantenimiento).toLocaleString('es-PE', { minimumFractionDigits: 2 })}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="table w-full">
                                        <thead><tr className="text-gray-400 uppercase text-[10px]"><th>Estado</th><th>Doc</th><th>Adjunto</th><th>Notas</th></tr></thead>
                                        <tbody>
                                            {documentosList.map((doc) => (
                                                <tr key={doc.key} className="hover:bg-gray-50 border-b border-gray-50">
                                                    <td className="text-center cursor-pointer font-black" onClick={() => toggleEstado(doc.key)}>{getIconoSemaforo(estadosDocs[doc.key])}</td>
                                                    <td className="font-black text-gray-700 text-sm">{doc.label}</td>
                                                    <td>
                                                        {documentosUrls[doc.key] ? (
                                                            <div className="flex items-center gap-2">
                                                                <a href={documentosUrls[doc.key].startsWith('http') ? documentosUrls[doc.key] : `${BACKEND_URL}${documentosUrls[doc.key]}`} target="_blank" className="btn btn-xs btn-primary text-white" title="Ver Documento"><FaEye/></a>
                                                                <label className="btn btn-xs btn-outline btn-primary cursor-pointer" title="Reemplazar PDF"><FaFileUpload/><input type="file" accept=".pdf" className="hidden" onChange={(e) => e.target.files?.[0] && handleSubirPdfAuditoria(doc.key, e.target.files[0])} /></label>
                                                            </div>
                                                        ) : (
                                                            <label className="btn btn-xs btn-ghost text-indigo-600 cursor-pointer"><FaFileUpload className="mr-1"/> Subir PDF<input type="file" accept=".pdf" className="hidden" onChange={(e) => e.target.files?.[0] && handleSubirPdfAuditoria(doc.key, e.target.files[0])} /></label>
                                                        )}
                                                    </td>
                                                    <td><textarea className="textarea textarea-bordered w-full h-10 text-xs font-black" value={observaciones[doc.key] || ''} onChange={(e) => setObservaciones({...observaciones, [doc.key]: e.target.value})}></textarea></td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
            ...
            <div className="lg:col-span-1 space-y-6">
                <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 sticky top-24 font-black">
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-2 italic">Precio Inmueble</p>
                    {!esProyecto ? (
                    ...
                    {propietarios.length > 0 && (
                        <div className="space-y-4 mb-6">
                            <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-2xl border border-blue-100 font-black"><div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white"><FaUserTie size={18} /></div><div className="flex flex-col"><span className="text-[9px] text-blue-500 font-black uppercase tracking-tight leading-none mb-1">Titular</span><span className="text-sm font-black text-slate-800 truncate max-w-[180px]" title={propietarios[0].nombre}>{propietarios[0].nombre}</span></div></div>
                    ...
                    {linksDisponibles.length > 0 && (
                        <div className="space-y-3 mb-6 pt-4 border-t border-gray-50">
                            <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-2">Recursos y Presentación</p>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                        <div className="avatar placeholder"><div className="bg-indigo-600 text-white rounded-2xl w-14 h-14 flex items-center justify-center text-xl font-bold font-black">{propiedad.asesor?.charAt(0) || 'S'}</div></div>
                    ...
                    {propietarios.length > 1 && (
                        <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            <div className="space-y-2">
                                {propietarios.slice(1).map((p: any) => (<p key={p.id} className="font-black text-gray-800 text-sm flex items-center gap-2 font-black"><FaUserTie className="text-gray-400" />{p.nombre}</p>))}
                            </div>
                        </div>
                    )}
