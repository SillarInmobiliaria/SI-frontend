...
import { useForm, useFieldArray } from 'react-hook-form'; // Agregamos useFieldArray
import { useRouter } from 'next/navigation';
import Navbar from '../../../components/Navbar';
import { useInmobiliariaStore } from '../../../store/useInmobiliariaStore';
import { createPropiedad, getCartera } from '../../../services/api'; // Importamos getCartera
...
  FaShieldAlt, FaTools, FaCalendarAlt, FaBuilding // Agregamos iconos
} from 'react-icons/fa';
...
        renovable: 'no',
        incluyeIgv: 'no',
        tipologias: [{ precio: '', areaConstruida: '', nombre: '' }] // Mínimo 1 al iniciar
    }
  });
...
            setAsesoresDB(data);
            ...
            // Jalamos las constructoras desde la cartera (PJ)
                   ...
                   <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 shadow-inner">
                            ...
                            <div className="space-y-3">
                                {fields.map((field, index) => (
                                    <div key={field.id} className="flex gap-3 items-center bg-white p-3 rounded-xl border shadow-sm font-bold">
                                        <div className="flex-1">
                                            <input type="number" step="0.01" {...register(`tipologias.${index}.precio` as const)} placeholder="Precio" className="input input-sm input-bordered w-full bg-white text-gray-800" />
                                        </div>
                                        <div className="flex-1">
                                            <input type="number" step="0.01" {...register(`tipologias.${index}.areaConstruida` as const)} placeholder="Área Const. m²" className="input input-sm input-bordered w-full bg-white text-gray-800" />
                                        </div>
                                        <div className="flex-[1.5]">
                                            <input type="text" {...register(`tipologias.${index}.nombre` as const)} placeholder="Tipología (Ej: 3 Dorm + Terraza)" className="input input-sm input-bordered w-full bg-white text-gray-800" />
                                        </div>
                                        {fields.length > 1 && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="form-control">
                                <label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide"><FaCalendarAlt className="mr-1"/> Fecha de Inicio</label>
                                <input type="date" {...register('fechaInicioProyecto')} className="input input-bordered w-full bg-white"/>
                            </div>
                            <div className="form-control">
                                <label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide">Tiempo Ejecución</label>
                                <input type="text" {...register('tiempoEjecucion')} placeholder="Ej: 18 meses" className="input input-bordered w-full bg-white"/>
                            </div>
                            <div className="form-control relative">
                                <label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide"><FaBuilding className="mr-1"/> Constructora (PJ)</label>
                                <div className="flex items-center">
                                    <FaSearch className="absolute left-3 text-gray-400 z-10 text-xs"/>
                                    <input type="text" className="input input-bordered w-full bg-white pl-10 text-sm" placeholder="Buscar empresa..." value={busquedaConstructora} onChange={(e) => { setBusquedaConstructora(e.target.value); setMostrarSugerenciasConstructora(true); }} onFocus={() => setMostrarSugerenciasConstructora(true)}/>
                                </div>
                                {mostrarSugerenciasConstructora && busquedaConstructora.length > 0 && (
                                    <div className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto mt-1">
                                        {constructorasDB.filter(c => c.empresa.toLowerCase().includes(busquedaConstructora.toLowerCase())).map((c) => (
                                            <div key={c.id} className="p-3 hover:bg-indigo-50 cursor-pointer border-b border-gray-100 flex flex-col" onClick={() => seleccionarConstructora(c)}>
                                                <span className="font-bold text-indigo-700 text-xs uppercase">{c.empresa}</span>
                                                <span className="text-[10px] text-gray-400">RUC: {c.ruc}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="form-control"><label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide">Precio *</label>
                            <div className="flex shadow-sm rounded-lg overflow-hidden border border-gray-300">
                                ...
                                <input type="number" step="0.01" {...register('precio', {required: !esProyecto})} className="input w-full bg-white font-bold text-lg focus:outline-none border-none text-gray-800" placeholder="0.00"/>
                            </div>
                        </div>
