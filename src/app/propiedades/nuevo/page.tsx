      ...
      <main className="container mx-auto px-6 max-w-5xl mt-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="form-control relative">
                        ...
                        <div className="flex items-center">
                            ...
                            <input type="text" className="input input-bordered w-full bg-white pl-9 text-sm font-black text-indigo-700 uppercase" placeholder="Buscar distrito..." value={busquedaUbicacion} onChange={(e) => { setBusquedaUbicacion(e.target.value); setMostrarSugerenciasUbi(true); }} onFocus={() => setMostrarSugerenciasUbi(true)}/>
                        </div>
                        {mostrarSugerenciasUbi && busquedaUbicacion.length > 0 && (
                            <div className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto mt-1 font-bold text-xs uppercase text-slate-700">
                                {distritosArequipa.filter(d => d.toLowerCase().includes(busquedaUbicacion.toLowerCase())).map((d, i) => (
                                    <div key={i} className="p-3 hover:bg-indigo-50 cursor-pointer border-b border-gray-100" onClick={() => seleccionarDistrito(d)}>{d}</div>
                                ))}
                            </div>
                        )}
                    </div>
                ...
                <div className="form-control mb-6"><label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide">Dirección Exacta</label><input {...register('direccion')} className="input input-bordered w-full bg-white font-black text-indigo-700"/></div>
                ...
                {esProyecto ? (
                    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 shadow-inner font-black">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button type="button" onClick={() => append({ precio: '', areaConstruida: '', nombre: '' })} className="btn btn-xs btn-primary gap-1 font-black"><FaPlus/> Añadir Tipología</button>
                            </div>
                            <div className="space-y-3 font-bold">
                                {fields.map((field, index) => (
                                    <div key={field.id} className="flex gap-3 items-center bg-white p-3 rounded-xl border shadow-sm">
                                        ...
                                        <div className="flex-1"><input type="number" step="0.01" {...register(`tipologias.${index}.areaConstruida` as const)} placeholder="Área Const. m²" className="input input-sm input-bordered w-full bg-white font-bold" /></div>
                                        <div className="flex-[1.5]"><input type="text" {...register(`tipologias.${index}.nombre` as const)} placeholder="Tipología (Ej: 3 Dorm + Terraza)" className="input input-sm input-bordered w-full bg-white font-bold" /></div>
                                        {fields.length > 1 && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-black">
                            <div className="form-control"><label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide"><FaCalendarAlt className="mr-1"/> Fecha Inicio</label><input type="date" {...register('fechaInicioProyecto')} className="input input-bordered w-full bg-white font-black text-indigo-700"/></div>
                            <div className="form-control"><label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide">Tiempo Ejecución</label><input type="text" {...register('tiempoEjecucion')} placeholder="Ej: 18 meses" className="input input-bordered w-full bg-white font-black text-indigo-700"/></div>
                            <div className="form-control relative"><label className="label font-bold text-gray-600 text-[10px] uppercase tracking-wide"><FaBuilding className="mr-1"/> Constructora (PJ)</label>
                                <div className="flex items-center"><FaSearch className="absolute left-3 text-gray-400 z-10 text-xs"/><input type="text" className="input input-bordered w-full bg-white pl-10 text-sm font-black text-indigo-700 uppercase" placeholder="Buscar empresa..." value={busquedaConstructora} onChange={(e) => { setBusquedaConstructora(e.target.value); setMostrarSugerenciasConstructora(true); }} onFocus={() => setMostrarSugerenciasConstructora(true)}/></div>
                                {mostrarSugerenciasConstructora && busquedaConstructora.length > 0 && (
                                    <div className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto mt-1 font-bold text-xs uppercase text-slate-700">
                                        {constructorasDB.filter(c => c.empresa.toLowerCase().includes(busquedaConstructora.toLowerCase())).map((c) => (
                                            <div key={c.id} className="p-3 hover:bg-indigo-50 cursor-pointer border-b border-gray-100 flex flex-col" onClick={() => seleccionarConstructora(c)}><span className="text-indigo-700 font-black">{c.empresa}</span><span className="text-[10px] text-gray-400 font-black">RUC: {c.ruc}</span></div>
                                        ))}
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 font-black">
                        ...
                        <div className="form-control"><label className="label font-bold text-gray-600 text-[10px] uppercase">ÁREA TOTAL (m²)</label><input type="number" step="0.01" {...register('area')} className="input input-bordered w-full bg-white font-black text-indigo-700" placeholder="0.00"/></div>
                        <div className="form-control"><label className="label font-bold text-gray-600 text-[10px] uppercase">ÁREA CONSTRUIDA (m²)</label><input type="number" step="0.01" {...register('areaConstruida')} className="input input-bordered w-full bg-white font-black text-indigo-700" placeholder="0.00"/></div>
                    </div>
                )}

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 font-black">
                    {/* VIGILANCIA: OCULTA SÓLO SI ES PROYECTO */}
                    {!esProyecto && (
                        <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl font-black">
                            <div className="form-control">
                                <label className="label text-emerald-800 text-[10px] uppercase font-black"><FaShieldAlt className="mr-1"/> ¿Pago de Vigilancia?</label>
                    ...
                    {/* MANTENIMIENTO: VISIBLE SIEMPRE PARA ALQUILER */}
                    {modalidadActual === 'Alquiler' && (
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl font-black">
                            <div className="form-control">
                                <label className="label font-bold text-blue-800 text-[10px] uppercase "><FaTools className="mr-1"/> ¿Mantenimiento?</label>
                                <div className="flex gap-4 mb-3">
                                ...
                                {tieneMantenimiento === 'si' && (<div className="flex shadow-sm rounded-lg overflow-hidden border border-blue-200"><select {...register('monedaMantenimiento')} className="bg-white px-3 font-bold text-blue-700 outline-none border-r border-blue-200 text-xs "><option value="PEN">S/</option><option value="USD">$</option></select><input type="number" step="0.01" {...register('mantenimiento')} className="input w-full bg-white font-bold focus:outline-none border-none text-gray-800 h-10 font-black" placeholder="Monto Mensual"/></div>)}
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 font-black">
                ...
                <div className="form-control font-black"><label className="label font-bold text-gray-700 text-xs uppercase"><FaListUl className="text-blue-500 mr-2 font-black"/> Distribución Detallada</label><textarea {...register('detalles')} className="textarea textarea-bordered h-40 bg-gray-50 focus:bg-white text-sm font-black" placeholder="Detalle piso por piso..."></textarea></div>
            </div>

            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 font-black">
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-6 flex items-center gap-2 border-b pb-2"><FaGavel className="text-blue-500 font-black"/> 4. DATOS LEGALES</h3>
